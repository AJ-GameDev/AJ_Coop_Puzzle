    ...
    public class TreeManager
    {
        ...
        public void LoadTreesFromTerrain()
        {
            ...
            foreach (var terrain in Terrain.activeTerrains)
                ...
                {
                    if (terrain.transform.position.x < minX) minX = terrain.transform.position.x;
                    if (terrain.transform.position.z < minZ) minZ = terrain.transform.position.z;
            ...
            {
                var terrainBounds = new Rect(minX, minZ, maxX - minX, maxZ - minZ);

                m_terrainTreeLocations = new Quadtree<TreeStruct>(terrainBounds);
                m_terrainTrees = new List<TreePrototype>(sampleTerrain.terrainData.treePrototypes);

                foreach (var terrain in Terrain.activeTerrains)
                {
                    var terrainOffsetX = terrain.transform.position.x;
                    var terrainOffsetY = terrain.transform.position.y;
                    var terrainOffsetZ = terrain.transform.position.z;
                    var terrainWidth = terrain.terrainData.size.x;
                    var terrainHeight = terrain.terrainData.size.y;
                    var terrainDepth = terrain.terrainData.size.z;
                    var terrainTreeInstances = terrain.terrainData.treeInstances;
                    for (var treeIdx = 0; treeIdx < terrainTreeInstances.Length; treeIdx++)
                    {
                        var treeInstance = terrainTreeInstances[treeIdx];
                        var newTree = new TreeStruct();
        ...
        {
            if (m_terrainTreeLocations == null) return;
        ...
        {
            if (m_terrainTreeLocations == null) return 0;
            var query = new Rect(position.x - range, position.z - range, range * 2f, range * 2f);
...
#pragma warning disable 414
        private List<TreePrototype> m_terrainTrees = new();
        private Quadtree<TreeStruct> m_terrainTreeLocations = new(new Rect(0, 0, 10f, 10f));
