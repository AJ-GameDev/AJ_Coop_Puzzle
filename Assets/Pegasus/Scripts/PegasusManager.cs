...
namespace Pegasus
{
    ...
    public class PegasusManager : MonoBehaviour
    {
        ...
        ///     The POI list
        /// </summary>
        public List<PegasusPoi> m_poiList = new();
        ...
        /// </summary>
        public bool m_displayDebug; //Set to true if we want debug messages
        ...
        public bool m_canUpdateNow; //Used to trigger when the camera and targetLocation can be updated
        ...
        public PegasusManager m_nextPegasus; //Used when single shot is assigned and end action is play next pegasus
        ...
        private void Start()
        {
            ...
            if (m_target == null)
            {
                ...
                if (m_displayDebug) Debug.Log("Assigning main camera to target : " + Camera.main.name);
                m_target = Camera.main.gameObject;
            }

            //Grab the defaults if none have been defined
            if (m_defaults == null) SetDefaults();
            ...
            if (m_autoStartAtRuntime) StartFlythrough();
        }
        ...
        {
            if (m_displayDebug) Debug.Log("Initialising flythrough...");
            ...
            for (idx = 0; idx < m_poiList.Count; idx++) m_poiList[idx].Initialise(false);
        ...
        {
            if (m_displayDebug) Debug.Log("Restarting flythrough...");
        ...
        public void UpdateFlythroughMetaData()
        {
            m_totalDuration = TimeSpan.Zero;
            m_totalDistance = 0f;
            for (var idx = 0; idx < m_poiList.Count; idx++)
            {
                m_poiList[idx].m_segmentStartTime = new TimeSpan(m_totalDuration.Ticks);
                m_totalDuration += m_poiList[idx].m_segmentDuration;
                m_totalDistance += m_poiList[idx].m_segmentDistance;
        ...
        public void UpdateSegmentWithDependencies(PegasusPoi segment)
        {
            ...
            if (m_flythroughType == PegasusConstants.FlythroughType.SingleShot)
            {
                var idx = segment.m_segmentIndex;
        ...
        private void SafeInitialise(int idx, bool wrap, bool updateSegments)
        {
            if (!wrap)
            {
                if (idx >= 0 && idx < m_poiList.Count) m_poiList[idx].Initialise(updateSegments);
            }
            else
            {
        ...
        public void StartFlythrough(bool fullInitialise = false)
        {
            ...
            if (m_displayDebug) Debug.Log("Starting flythrough..");
        ...
        {
            if (m_displayDebug) Debug.Log("Resuming flythrough");
        ...
        {
            if (m_displayDebug) Debug.Log("Pausing flythrough");
        ...
        {
            if (m_displayDebug) Debug.Log("Stopping flythrough");
            m_currentState = PegasusConstants.FlythroughState.Stopped;
        ...
        public void ChangeFramerate(PegasusConstants.TargetFrameRate newRate)
        {
            ...
            if (m_targetFramerateType == PegasusConstants.TargetFrameRate.LeaveAlone) return;
        ...
        public void SetAutoRoll(bool autoRoll)
        {
            for (var idx = 0; idx < m_poiList.Count; idx++)
            {
                ...
                if (!autoRoll) m_poiList[idx].transform.localEulerAngles = Vector3.zero;
            }
        ...
        {
            for (var idx = 0; idx < m_poiList.Count; idx++) m_poiList[idx].m_isSelected = false;
            if (poi != null) poi.m_isSelected = true;
        }
        ...
        public void MovePoiLookat(PegasusPoi poi, Vector3 movement)
        {
            ...
            var lookAtLocation = GetValidatedLookatPosition(poi.m_lookatLocation + movement, poi.m_heightCheckType);
        ...
        public void MoveTargetTo(float percent)
        {
            ...
            PegasusPoi poi;
            var targetPoint = percent * m_totalDistance;
            var segmentStart = 0f;
            var segmentEnd = 0f;
            for (var idx = 0; idx < m_poiList.Count; idx++)
            {
            ...
            out float velocity)
        {
            ...
            else
            {
                ...
                if (targetPoint < 0f) targetPoint += m_totalDistance;
            }
        ...
        public void CalculateTargetAtPercent(float percent, out Vector3 position, out Quaternion rotation,
            out float velocity)
        {
            var targetPoint = Mathf.Clamp(percent, 0f, 100f) * m_totalDistance;
        ...
        public void StepTargetBackward(float distMeters)
        {
            ...
            if (m_totalDistanceTravelled < 0) m_totalDistanceTravelled = 0f;
            MoveTargetTo(m_totalDistanceTravelled / m_totalDistance);
        }
        ...
        {
            m_totalDistanceTravelled += distMeters;
            if (m_totalDistanceTravelled > m_totalDistance) m_totalDistanceTravelled = m_totalDistance;
            MoveTargetTo(m_totalDistanceTravelled / m_totalDistance);
        }
        ...
        public void DeleteDebugObjects()
        {
            ...
            {
                poi = m_poiList[idx];
                while (poi.transform.childCount > 0) DestroyImmediate(poi.transform.GetChild(0).gameObject);
            }
        }
                ...
                PegasusConstants.PoiHeightCheckType.ManagerSettings)
        {
            ...
            var heightCheckType = m_heightCheckType;
            ...
            if (heightCheckType == PegasusConstants.HeightCheckType.None) return source;
            ...
            var t = GetTerrain(source);
            if (t != null)
            {
                var height = t.SampleHeight(source);
                if (height + m_minHeightAboveTerrain > source.y) source.y = height + m_minHeightAboveTerrain;
            }
                ...
                PegasusConstants.PoiHeightCheckType.ManagerSettings)
        {
            ...
            if (heightCheckType == PegasusConstants.HeightCheckType.Collision)
            {
                RaycastHit hit;
                if (Physics.Raycast(new Vector3(source.x, source.y + m_collisionHeightOffset, source.z), Vector3.down,
                        out hit, 10000f)) source.y = hit.point.y + m_minHeightAboveTerrain;
                return source;
            }

            var t = GetTerrain(source);
            if (t != null) source.y = t.SampleHeight(source) + m_minHeightAboveTerrain;
            return source;
        }
                ...
                PegasusConstants.PoiHeightCheckType.ManagerSettings)
        {
            ...
            {
                var height = t.SampleHeight(source);
                if (height > source.y) source.y = height;
            }
                ...
                PegasusConstants.PoiHeightCheckType.ManagerSettings)
        {
            ...
            if (heightCheckType == PegasusConstants.HeightCheckType.Collision)
            {
                RaycastHit hit;
                if (Physics.Raycast(new Vector3(source.x, source.y + m_collisionHeightOffset, source.z), Vector3.down,
                        out hit, 2000f)) source.y = hit.point.y;
                return source;
            }

            var t = GetTerrain(source);
            if (t != null) source.y = t.SampleHeight(source);
            return source;
        }
        ...
        public float GetValidatedLookatHeightRelativeToMinimum(Vector3 source,
            PegasusConstants.PoiHeightCheckType heightCheckOverride =
                PegasusConstants.PoiHeightCheckType.ManagerSettings)
        {
            var minPosition = GetLowestLookatPosition(source, heightCheckOverride);
        ...
        private void LateUpdate()
        {
            ...
            if (m_currentState == PegasusConstants.FlythroughState.Paused) m_lastUpdateTime = DateTime.Now;

            //Exit if we are not in running mode
            if (m_currentState != PegasusConstants.FlythroughState.Started) return;
        ...
        /// <summary>
        ///     Perform the calculations required for the next update - will trigger an update in the LateUpdate method to apply it
        ///     to the scene
        /// </summary>
        private void CalculateFlythroughUpdates()
        {
            ...
            if (m_currentSegment != null)
            {
                ...
                {
                    m_frameUpdateTime = (DateTime.Now - m_lastUpdateTime).Milliseconds / 1000f;
                    m_lastUpdateTime = DateTime.Now;
                }
...
//                }
        ...
        {
            var newPoiGo = new GameObject("POI " + m_poiList.Count);
            newPoiGo.transform.parent = transform;
            newPoiGo.transform.position = targetLocation;

            var newPoi = newPoiGo.AddComponent<PegasusPoi>();
            newPoi.m_manager = this;
            newPoi.m_lookatLocation = lookatLocation;

            if (targetLocation != lookatLocation) newPoi.m_lookatType = PegasusConstants.LookatType.Target;
        ...
        {
            if (m_poiList.Count < 1) return null;
            return m_poiList[0];
        }
        ...
        {
            if (m_poiList.Count == 0 || poiIndex < 0 || poiIndex >= m_poiList.Count) return null;
        ...
        public PegasusPoi GetPrevPOI(PegasusPoi currentPoi, bool wrap = true)
        {
            if (currentPoi != null)
            {
                if (currentPoi.m_segmentIndex > 0) return m_poiList[currentPoi.m_segmentIndex - 1];
        ...
        public PegasusPoi GetNextPOI(PegasusPoi currentPoi, bool wrap = true)
        {
            if (currentPoi != null)
            {
                if (currentPoi.m_segmentIndex < m_poiList.Count - 1) return m_poiList[currentPoi.m_segmentIndex + 1];
