...
namespace Pegasus
{
    ...
    public class PegasusPoi : MonoBehaviour
    {
        ...
        [HideInInspector] public int m_segmentIndex = 0; //The index of this segment in the overal flythrough
...
//                    Vector3 posForward = posOrigin + Quaternion.Euler(transform.localEulerAngles) * (hdgPrevOrigin * distOriginTarget);
        ...
        private void DrawGizmos(bool isSelected)
        {
            ...
            if (Selection.activeGameObject != null &&
                Selection.activeGameObject.GetInstanceID() != this.gameObject.GetInstanceID())
            {
                m_isSelected = false;
            }
#endif
        ...
        public void UpdateSegment()
        {
            ...
            if (m_manager.m_flythroughType == PegasusConstants.FlythroughType.SingleShot &&
                m_manager.GetNextPOI(this, false) == null)
            {
                return;
            }
            ...
            if (m_nextPoi != null)
            {
                     ...
                     measurement++)
                {
                    ...
                    //For debugging
                    if (ApproximatelyEqual(minMeasurementDistance, 0f) ||
                        (measurementDistance < minMeasurementDistance))
                    {
                        minMeasurementDistance = measurementDistance;
                    }

                    if (ApproximatelyEqual(maxMeasurementDistance, 0f) ||
                        (measurementDistance > maxMeasurementDistance))
                    {
                ...
                float
                    expectedStepDistance =
                        1f / (float)stepsPerMeter; //We want an exact multiple that gets to the whole length, but is nearest to 1 / stepsPerMeter
