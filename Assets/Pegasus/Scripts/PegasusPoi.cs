...
namespace Pegasus
{
    ...
    {
        [Tooltip(
            "The type of POI. Auto generated POI's are subject to automatic deletion. Manual POI's will always be kept.")]
        ...
        [Tooltip(
            "The start speed type for this segment. The segement will always start at this speed and ease to the next segments start speed.")]
        public PegasusConstants.SpeedType m_startSpeedType = PegasusConstants.SpeedType.Medium;

        [Tooltip(
            "The actual start speed for this segment. Speed varies between stat speed for this segment and start speed for next segment.")]
        public float m_startSpeed = 1f;

        [HideInInspector] public PegasusConstants.EasingType m_rotationEasingType = PegasusConstants.EasingType.Linear;

        [HideInInspector]
        public PegasusConstants.EasingType m_velocityEasingType = PegasusConstants.EasingType.EaseInOut;

        [HideInInspector] public PegasusConstants.EasingType m_positionEasingType = PegasusConstants.EasingType.Linear;

        [HideInInspector] public PegasusManager m_manager; //The manager which created this poi and owns it

        [HideInInspector] public bool m_alwaysShowGizmos = true; //Whether we should show gizmos even when not selected

        [HideInInspector] public float m_segmentDistance; //The actual travel distance of this segment based on spline

        [HideInInspector] public int m_segmentIndex; //The index of this segment in the overal flythrough

        [HideInInspector] public bool m_isFirstPOI = true; //Whether or not we are the first POI

        [HideInInspector] public bool m_isLastPOI = true; //Whether or not we are the last POI

        [HideInInspector] public PegasusPoi m_prevPoi; //The previous POI, will initially be set this

        [HideInInspector] public PegasusPoi m_nextPoi; //The next POI, will initially be set to this

        [HideInInspector]
        public List<Vector3> m_poiSteps = new(); //The steps that this POI segment will traverse through

        [HideInInspector] public bool m_isSelected; //Whether or not this poi is selected - draws a different line

        [HideInInspector] public List<TriggerBase> m_poiTriggers = new(); //The triggers associated with this poi

        [HideInInspector] public bool m_autoRollOn; //Whether or not this POI should calculate banking automatically
        ...
        [HideInInspector]
        public TimeSpan
            m_segmentStartTime = TimeSpan.Zero; //The time after start time that this segment will start playing
        ...
        public bool IsSameObject(PegasusPoi poi)
        {
            ...
            if (GetInstanceID() == poi.GetInstanceID()) return true;
            return false;
        }
        ...
        public void Initialise(bool updateSegments)
        {
            ...
            if (m_prevPoi == null || m_nextPoi == null) return;
            ...
            switch (m_lookatType)
            {
                case PegasusConstants.LookatType.Path:
                    ...
                    GetRelativeOffsets(transform.position, m_lookatLocation, out m_lookAtDistance, out m_lookAtHeight,
                        out m_lookAtAngle);
                    break;
            ...
            if (m_autoRollOn)
                if (m_manager.m_flythroughType == PegasusConstants.FlythroughType.Looped ||
                    (!m_isFirstPOI && !m_isLastPOI))
                {
                    var posPrev = m_prevPoi.transform.position;
                    var posOrigin = transform.position;
                    var posTarget = m_nextPoi.transform.position;
                    ...
                    var hdgPrevOrigin = posOrigin - posPrev;
                    ...
                    var qOriginForward = Quaternion.identity;
                    if (hdgPrevOrigin != Vector3.zero) qOriginForward = Quaternion.LookRotation(hdgPrevOrigin);

                    //Origin to target
                    var hdgOriginTarget = posTarget - posOrigin;
                    var qOriginTarget = Quaternion.identity;
                    if (hdgOriginTarget != Vector3.zero) qOriginTarget = Quaternion.LookRotation(hdgOriginTarget);

                    //Scale things
                    var angle = qOriginTarget.eulerAngles.y - qOriginForward.eulerAngles.y;
                    var scaledSpeed =
                        Mathf.Clamp(m_startSpeed, PegasusConstants.SpeedReallySlow, m_manager.m_autoRollMaxSpeed) /
                        m_manager.m_autoRollMaxSpeed;
                    var scaledAngle = angle;
                    if (angle < 0f)
                        scaledAngle = Mathf.Clamp(angle, -90f, 0f) / 90f;
                    else
                        scaledAngle = Mathf.Clamp(angle, 0f, 90f) / 90f;
                    var z = scaledAngle * scaledSpeed * m_manager.m_autoRollMaxAngle * -1f;
            ...
            var rotationDir = m_lookatLocation - transform.position;
            if (rotationDir != Vector3.zero)
                m_rotationStart = Quaternion.LookRotation(m_lookatLocation - transform.position) *
            ...
            if (updateSegments) UpdateSegment();
        ...
        {
            for (var idx = 0; idx < m_poiTriggers.Count; idx++) m_poiTriggers[idx].OnStart(this);
        }
        ...
        {
            for (var idx = 0; idx < m_poiTriggers.Count; idx++) m_poiTriggers[idx].OnUpdate(this, progress);
        }
        ...
        {
            for (var idx = 0; idx < m_poiTriggers.Count; idx++) m_poiTriggers[idx].OnEnd(this);
        }

        #region Routines influenced by terrain

        /// <summary>
        ///     Get the offsets of targetDistance, targetHeight relative to ground, and targetAngle of y rotation from source
        ///     position to target position, as if they were on the same plane
        /// </summary>
        ...
        public void GetRelativeOffsets(Vector3 source, Vector3 target, out float targetDistance, out float targetHeight,
            out float targetAngle)
        {
            targetHeight = m_manager.GetValidatedLookatHeightRelativeToMinimum(target, m_heightCheckType);
            var planarTargetPosition = new Vector3(target.x, source.y, target.z);
            targetDistance = Vector3.Distance(source, planarTargetPosition);

            var targetDirection = source - target;
        ...
        private void DrawGizmos(bool isSelected)
        {
            ...
            if (transform.parent.childCount != m_manager.m_poiList.Count) m_manager.InitialiseFlythrough();

#if UNITY_EDITOR
            //Make a check to see if we are selected - disable if not
            if (Selection.activeGameObject != null &&
                Selection.activeGameObject.GetInstanceID() != gameObject.GetInstanceID()) m_isSelected = false;
#endif

            //Now draw the gizmos

            //Segment path spline
            var velRange = PegasusConstants.SpeedStratospheric - PegasusConstants.SpeedReallySlow;
            if (m_nextPoi != null)
            {
                //See if we need to draw the segments for this
                var drawSegments = true;
                ...
                if (drawSegments)
                {
                    ...
                    for (var pct = inc; pct <= 1.02f; pct += inc)
                    {
                        //pos2 = CalculatePositionSpline(pct);
                        pos2 = CalculatePositionLinear(pct);
                        vel2 = CalculateVelocity(pct);
                        if (m_isSelected)
                            Gizmos.color = Color.magenta * Color.Lerp(Color.cyan, Color.red,
                                (vel1 + vel2 / 2f - PegasusConstants.SpeedReallySlow) / velRange);
                        else
                            Gizmos.color = Color.Lerp(Color.cyan, Color.red,
                                (vel1 + vel2 / 2f - PegasusConstants.SpeedReallySlow) / velRange);
            ...
            */
            ...
            if (m_lookatType == PegasusConstants.LookatType.Target)
            {
                ...
                Gizmos.color = Color.Lerp(Color.cyan, Color.red,
                    (m_startSpeed - PegasusConstants.SpeedReallySlow) / velRange);
            ...
            {
                var nextPoi = m_manager.m_nextPegasus.GetFirstPOI();
                if (nextPoi != null) Gizmos.DrawLine(transform.position, nextPoi.transform.position);
            }
        ...
        public void UpdateSegment()
        {
            ...
            if (!m_isFirstPOI) m_segmentStartTime = m_prevPoi.m_segmentStartTime + m_prevPoi.m_segmentDuration;
            m_poiSteps.Clear();

            if (m_manager.m_flythroughType == PegasusConstants.FlythroughType.SingleShot &&
                m_manager.GetNextPOI(this, false) == null) return;
            ...
            if (m_nextPoi != null)
            {
                ...
                var stepsPerMeter = 3;
                var measurementsPerMeter =
                    stepsPerMeter *
                    20; //Another magic multiplier - the more steps per meter the more measurements are required
                int measurement;
                var straightLineDistance = Vector3.Distance(transform.position, m_nextPoi.transform.position);
                var totalMeasurments = (int)Mathf.Ceil(measurementsPerMeter * straightLineDistance);
                var measurementIncrement = 1f / totalMeasurments;
                var measurementDistance = 0f;
                var steppedDistance = 0f;
                var minMeasurementDistance = 0f;
                var maxMeasurementDistance = 0f;
                var totalSteppedDistance = 0f;
                var minMeasuredStepDistance = 0f;
                var maxMeasuredStepDistance = 0f;

                pos1 = transform.position;
                for (measurement = 1, pct = 0f, minMeasurementDistance = 0f, maxMeasurementDistance = 0f;
                     measurement <= totalMeasurments;
                     measurement++)
                {
                ...
                if (m_segmentDistance < 2f) stepsPerMeter *= 3; //Arbitrar magic value
                ...
                var expectedStepDistance =
                    1f / stepsPerMeter; //We want an exact multiple that gets to the whole length, but is nearest to 1 / stepsPerMeter
                expectedStepDistance = m_segmentDistance / Mathf.Floor(m_segmentDistance / expectedStepDistance);

                pos1 = transform.position;
                m_poiSteps.Add(pos1);
                for (measurement = 1, pct = 0f, minMeasuredStepDistance = 0f, maxMeasuredStepDistance = 0f;
                     measurement <= totalMeasurments;
                     measurement++)
                {
                    pct += measurementIncrement;
                    ...
                    {
                        //For debugging
                        if (ApproximatelyEqual(minMeasuredStepDistance, 0f) ||
                            steppedDistance < minMeasuredStepDistance) minMeasuredStepDistance = steppedDistance;
                        if (ApproximatelyEqual(maxMeasuredStepDistance, 0f) ||
                            steppedDistance > maxMeasuredStepDistance) maxMeasuredStepDistance = steppedDistance;

                        //Lerp the intermediary steps to maintain distances
                        while (steppedDistance >= expectedStepDistance)
                        {
                            m_poiSteps.Add(Vector3.Lerp(m_poiSteps[m_poiSteps.Count - 1], pos2,
                                expectedStepDistance / steppedDistance));
        ...
        public void UpdateSegmentDuration()
        {
            ...
            m_segmentDuration = TimeSpan.Zero;
            var pct = 0f;
            var pos1 = CalculatePositionLinear(0f);
            var pos2 = Vector3.zero;
            var duration = 0f;
        ...
        /// </summary>
        /// <param name="percent">
        ///     The percentage through the segment that the calculations should be made for from 0 == 0% to 1 ==
        ///     100%
        /// </param>
        ...
        {
            return CatmullRom(m_prevPoi.transform.position, transform.position, m_nextPoi.transform.position,
                m_nextPoi.m_nextPoi.transform.position, percent);
        }

        /// <summary>
        ///     Calculate position based on the variables supplied. This must only be called after initialisation as it depends on
        ///     the initialisation setup to work.
        /// </summary>
        /// <param name="percent">
        ///     The percentage through the segment that the calculations should be made for from 0 == 0% to 1 ==
        ///     100%
        ...
        public Vector3 CalculatePositionLinear(float percent)
        {
            ...
            //Handle no data
            if (m_poiSteps.Count == 0) return Vector3.zero;

            //Handle only one item
            if (m_poiSteps.Count == 1) return m_poiSteps[0];

            var maxSegments = m_poiSteps.Count - 1;
            var firstSegment = (int)(percent * maxSegments);
            if (firstSegment == maxSegments) return m_poiSteps[firstSegment];

            var progress = percent * maxSegments - firstSegment;
        ...
        {
            return new Vector3(CalcCatmullRom(value1.x, value2.x, value3.x, value4.x, amount),
                CalcCatmullRom(value1.y, value2.y, value3.y, value4.y, amount),
                CalcCatmullRom(value1.z, value2.z, value3.z, value4.z, amount));
        }
        ...
        {
            return new Vector3(CalcHermite(value1.x, tangent1.x, value2.x, tangent2.x, amount),
                CalcHermite(value1.y, tangent1.y, value2.y, tangent2.y, amount),
                CalcHermite(value1.z, tangent1.z, value2.z, tangent2.z, amount));
        }
        ...
        public static float CalcHermite(float value1, float tangent1, float value2, float tangent2, float amount)
        {
            ...
            if (amount == 0f)
                ...
                result = (2 * v1 - 2 * v2 + t2 + t1) * sCubed + (3 * v2 - 3 * v1 - 2 * t1 - t2) * sSquared + t1 * s +
                         v1;
            return (float)result;
        }
