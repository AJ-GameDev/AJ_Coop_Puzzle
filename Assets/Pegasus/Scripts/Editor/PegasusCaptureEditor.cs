...
namespace Pegasus
{
    ...
    public class PegasusCaptureEditor : Editor
    {
        ...
        private void OnEnable()
        {
            ...
            {
                if (m_capture.m_path == null) m_capture.m_path = PegasusPath.CreatePegasusPath();
        ...
        public override void OnInspectorGUI()
        {
            ...
            var mainCamera = (Camera)EditorGUILayout.ObjectField(GetLabel("Target Camera"), m_capture.m_mainCamera,
                typeof(Camera), true);
            var path = (PegasusPath)EditorGUILayout.ObjectField(GetLabel("Pegasus Path"), m_capture.m_path,
                typeof(PegasusPath), false);
            var enableOnStart = EditorGUILayout.Toggle(GetLabel("Enable On Start"), m_capture.m_enableOnStart);
            var clearOnStart = EditorGUILayout.Toggle(GetLabel("Clear On Start"), m_capture.m_clearOnStart);
            var showReticule = EditorGUILayout.Toggle(GetLabel("Show Reticule"), m_capture.m_showReticule);

            if (path == null || path.m_path.Count == 0) GUI.enabled = false;
