...
namespace Pegasus
{
    ...
    public class PegasusPoiEditor : Editor
    {
        ...
        private static readonly Dictionary<string, string> m_tooltips = new()
        {
            ...
            {
                "Target",
                "Where the target should look. Path - the target will look along the path of the flythrough. Target - the target will look at a custom target."
            },
            ...
            {
                "Segment Time",
                "The amount of time that it will take to fly through this segment. Changing the camera speed will change this value."
            },
            { "Total Iime", "The total amount of time that it will take for the flythrough to complete." },
            {
                "Auto Roll",
                "Auto Roll will automatically roll or bank the POI on turns. If enabled then this POI's ROLL (Z) is automatically calculated based on its speed through the turn and on the Roll settings under Advanced in Pegasus Manager."
            }
        };
        ...
        private void OnSceneGUI()
        {
            ...
            if (m_poi == null || m_poi.m_manager == null) return;
            ...
            if (m_poi.transform.hasChanged)
            {
                ...
                m_poi.transform.position =
                    m_poi.m_manager.GetValidatedPoiPosition(m_poi.transform.position, m_poi.m_heightCheckType);
                m_poi.GetRelativeOffsets(m_poi.transform.position, m_poi.m_lookatLocation, out m_poi.m_lookAtDistance,
                    out m_poi.m_lookAtHeight, out m_poi.m_lookAtAngle);
            ...
            {
                var lookAtLocation = m_poi.m_manager.GetValidatedLookatPosition(
                    Handles.DoPositionHandle(m_poi.m_lookatLocation, Quaternion.identity), m_poi.m_heightCheckType);
                if (lookAtLocation != m_poi.m_lookatLocation)
                {
                    ...
                    m_poi.GetRelativeOffsets(m_poi.transform.position, m_poi.m_lookatLocation,
                        out m_poi.m_lookAtDistance, out m_poi.m_lookAtHeight, out m_poi.m_lookAtAngle);
                    m_poi.m_manager.UpdateSegmentWithDependencies(m_poi);
                    m_poi.m_manager.MoveTargetToPoi(m_poi);
                    EditorUtility.SetDirty(m_poi);
                    return;
            ...
            if (m_poi.m_manager.m_defaults == null) m_poi.m_manager.m_defaults = PegasusManagerEditor.GetDefaults();
            ...
            if (Event.current.control && Event.current.type == EventType.KeyDown)
            {
                ...
                if (Event.current.keyCode == m_poi.m_manager.m_defaults.m_keyLeft)
                {
                    ...
                    var movement = Quaternion.Euler(0F, SceneView.lastActiveSceneView.rotation.eulerAngles.y, 0f) *
                                   Vector3.left;
                ...
                if (Event.current.keyCode == m_poi.m_manager.m_defaults.m_keyPrevPoi)
                {
                    ...
                    var poi = m_poi.m_manager.GetPrevPOI(m_poi);
                    if (poi != null)
                    {
                ...
                if (Event.current.keyCode == m_poi.m_manager.m_defaults.m_keyNextPoi)
                {
                    ...
                    var poi = m_poi.m_manager.GetNextPOI(m_poi);
                    if (poi != null)
                    {
                        if (Selection.activeTransform != null) Selection.activeTransform = poi.transform;
                        if (SceneView.lastActiveSceneView != null)
            ...
            if (Event.current.control && Event.current.isMouse)
            {
                if (Event.current.type == EventType.MouseDown && Event.current.button == 0)
                {
                    ...
                    {
                        var newPoint = m_poi.m_manager.GetValidatedPoiPosition(hitInfo.point);
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.LabelField(
                "The yellow sphere will be flown through, and the blue sphere is what the camera is looking at. If the blue sphere is not shown then the camera will look along the path of the flythrough.\n\nClick on POI in scene then...\nCTRL + Keys - Move selected POI.\nSHIFT+CTRL + Keys - Move selected LookAt.\nSee PegasusDefaults for Keys.",
                m_wrapStyle);
            GUILayout.EndVertical();

            EditorGUI.BeginChangeCheck();

            var poiType = m_poi.m_poiType;

            GUILayout.Space(5);

            GUILayout.BeginVertical("Auto Roll", m_boxStyle);
            GUILayout.Space(20);
            var autoRoll = EditorGUILayout.Toggle(GetLabel("Auto Roll"), m_poi.m_autoRollOn);
            GUILayout.Space(3);
            GUILayout.EndVertical();
            ...
            GUILayout.Space(20);
            var lookatType =
                (PegasusConstants.LookatType)EditorGUILayout.EnumPopup(GetLabel("Target"), m_poi.m_lookatType);
            var lookAtAngle = m_poi.m_lookAtAngle;
            var lookAtDistance = m_poi.m_lookAtDistance;
            var lookAtHeight = m_poi.m_lookAtHeight;
            if (lookatType == PegasusConstants.LookatType.Path) GUI.enabled = false;
            ...
            GUILayout.Space(20);
            var speedType =
                (PegasusConstants.SpeedType)EditorGUILayout.EnumPopup(GetLabel("Type"), m_poi.m_startSpeedType);
            var startSpeed = m_poi.m_startSpeed;
            ...
            else
            {
                ...
                if (startSpeed < 0.0001f) startSpeed = 0.0001f;
            }
            ...
            GUILayout.Space(20);
            var heightCheckType =
                (PegasusConstants.PoiHeightCheckType)EditorGUILayout.EnumPopup(GetLabel("Min Height From"),
                    m_poi.m_heightCheckType);
            GUILayout.Space(3);
            GUILayout.EndVertical();
            ...
            EditorGUILayout.LabelField("Start Time",
                string.Format("{0}:{1:00}.{2:000}", m_poi.m_segmentStartTime.Minutes, m_poi.m_segmentStartTime.Seconds,
                    m_poi.m_segmentStartTime.Milliseconds));
            EditorGUILayout.LabelField("Segment Time",
                string.Format("{0}:{1:00}.{2:000}", m_poi.m_segmentDuration.Minutes, m_poi.m_segmentDuration.Seconds,
                    m_poi.m_segmentDuration.Milliseconds));
            EditorGUILayout.LabelField("Total Time",
                string.Format("{0}:{1:00}.{2:000}", m_poi.m_manager.m_totalDuration.Minutes,
                    m_poi.m_manager.m_totalDuration.Seconds, m_poi.m_manager.m_totalDuration.Milliseconds));
            GUILayout.Space(3);
            GUILayout.EndVertical();

            var velocityEasing = m_poi.m_velocityEasingType;
            var rotationEasing = m_poi.m_rotationEasingType;
            var positionEasing = m_poi.m_positionEasingType;
            ...
            {
                var poi = m_poi.m_manager.GetFirstPOI();
                if (poi != null)
                {
                    if (SceneView.lastActiveSceneView != null)
                        SceneView.lastActiveSceneView.pivot = poi.transform.position;
            ...
            {
                var poi = m_poi.m_manager.AddPoiBefore(m_poi);
                if (poi != null)
                {
                    if (Selection.activeTransform != null) Selection.activeTransform = poi.transform;
                    if (SceneView.lastActiveSceneView != null)
            ...
            {
                var poi = m_poi.m_manager.AddPoiAfter(m_poi);
                if (poi != null)
                {
                    if (Selection.activeTransform != null) Selection.activeTransform = poi.transform;
                    if (SceneView.lastActiveSceneView != null)
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                if (heightCheckType != m_poi.m_heightCheckType) m_poi.m_heightCheckType = heightCheckType;
                //m_poi.m_manager.UpdateSegmentWithDependencies(m_poi);
                if (!PegasusPoi.ApproximatelyEqual(m_poi.m_lookAtAngle, lookAtAngle))
                {
                    m_poi.m_lookAtAngle = lookAtAngle;
                    m_poi.m_lookatLocation = m_poi.transform.position + m_poi.transform.localRotation *
                        Quaternion.AngleAxis(m_poi.m_lookAtAngle, Vector3.up) *
                        new Vector3(0f, 0f, -m_poi.m_lookAtDistance);
                    m_poi.m_lookatLocation =
                        m_poi.m_manager.GetLowestLookatPosition(m_poi.m_lookatLocation, m_poi.m_heightCheckType);
