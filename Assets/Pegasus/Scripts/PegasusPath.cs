...
namespace Pegasus
{
    ...
    public class PegasusPath : ScriptableObject
    {
        ...
        public List<PegasusPoint> m_path = new();
        ...
        {
            var pp = CreateInstance<PegasusPath>();
#if UNITY_EDITOR
            var path = "Assets/PegasusPath.asset";
        ...
        public void AddPoint(GameObject go)
        {
            ...
            var location = go.transform.position;
            var rotation = go.transform.eulerAngles;
            PegasusPoint pp = null;
            if (m_path.Count > 0)
            {
                pp = m_path[m_path.Count - 1];
                if (pp.m_location == location && pp.m_rotationEuler != rotation) return;
            }
        ...
        public void CreatePegasusFromPath()
        {
            ...
            var pegasus = pegasusGo.AddComponent<PegasusManager>();
            pegasus.SetDefaults();
            ...
            {
                p = m_path[i];
                pegasus.AddPOI(p.m_location,
                    p.m_location + Quaternion.Euler(p.m_rotationEuler) * (Vector3.forward * 2f));
            }
